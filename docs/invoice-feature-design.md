...
### 2026-07-01（入金状況管理・一覧/編集UI整理）
- **入金状況管理**：`jo_invoices` に `paidDate`/`paidBy` 追加（**status とは独立**＝発行済＋送付済＋入金済を同時表現可）。一覧に入金状況列（未入金/入金済/**期限超過=赤**）＋入金フィルタ。手動消込＝未入金/期限超過タグのクリックで入金日入力→入金済、入金取消は⋯メニュー＋確認。API `markInvoicePaid`/`unmarkInvoicePaid`（admin/CSRF）。閲覧バナーに入金確認者・入金日。
- **void ガード**：入金済みの請求書は取消(void)不可（`paid_cannot_void`）→ **入金取消→取消** の手順を強制。入金済タグは表示専用（誤操作防止）。
- **一覧UI整理**：全フィルタ即時絞込＋絞込ボタン廃止（クリア追加）、請求月を年月ピッカー、行操作を「主要＋⋯メニュー」に集約。
- **編集UI**：アクションバーを上部固定(sticky)＋税込合計ライブ表示。請求対象月を年月ピッカー。明細の品目から消費税(allocationType=tax)を除外。PDFプレビュー説明文をmPDF対応に修正。
