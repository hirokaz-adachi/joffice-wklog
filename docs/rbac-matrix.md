...
## 整合性ガード（誤操作防止・2026-06-26）
admin の CRUD でも、次の操作は構造的にブロックする。詳細・エラーコード対訳は [production-auth-db-memo.md](./production-auth-db-memo.md) §9。
- **最後の有効 admin** の削除・無効化・降格は不可（全員締め出し防止）。**自分自身**の無効化・自己降格・自己削除も不可。
- **マスタ code（PK）は編集不可**（値参照に波及しないため／改番は削除→再追加）。
- **参照のあるマスタは削除不可**（工数・請求・顧客担当・ユーザー紐付け等。内訳を表示し、停止は `isActive=0` を案内）。tasks の工程は本体と一緒に削除。
- **staffCode は1ユーザー一意**（`uq_user_staff`）。
...
## 更新履歴
### 2026-06-26（整合性ガード追加）
- admin CRUD に誤操作防止ガードを実装（最後のadmin保護・自己無効化/降格防止・マスタcode不変・参照あり削除ブロック・staffCode一意）。エラーは具体メッセージで返す。詳細は production-auth-db-memo §9。
### 2026-06-25（新規作成）
